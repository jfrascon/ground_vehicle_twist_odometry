  ...
  class GroundVehicleTwistOdometry: public rclcpp::Node
  {
    ...
    public:
      GroundVehicleTwistOdometry(const std::string& node_name,
        ...
        twist_sub_{create_subscription<twist_type>("twist",
                                                   10,
                                                   std::bind(&GroundVehicleTwistOdometry::init_cb,
                                                             this,
                                                             std::placeholders::_1))},
        odom_pub_{create_publisher<nav_msgs::msg::Odometry>("odom", 10)},
        tf_pub_{*this},
        reset_srv_{create_service<std_srvs::srv::Empty>("reset_odom",
       ...
       * timestamped_twist is false).
       * The t_prev_ variable is then used to compute dt in the regular twist callback.
       * Then, the twist subscription is replaced with a new one
      ...
      void init_cb(const typename twist_type::ConstSharedPtr msg)
      {
        ...
        twist_sub_.reset();
        twist_sub_ = this->create_subscription<twist_type>("twist",
                                                           10,
                                                           std::bind(&GroundVehicleTwistOdometry::twist_cb,
                                                                     this,
                                                                     std::placeholders::_1));
      }
